...
<head><meta http-equiv="Content-Type" content="text/html; charset=utf-8">
    ...
    <div class="row main-top-row">
        <div class="col-md-12">
            ...
            @endif
            <form class="form-horizontal" method="POST" action="{{ route('building_document.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-8">
                        <h3 class="tile-title">Add New Document</h3>
                        @foreach ($errors->all() as $error)
                            <div class="alert alert-danger">{{ $error }}</div>
                        @endforeach
                        <div class="form-group">
                            <label>Documents</label>
                            <input id="documents" value="{{ old('documents')}}" type="text" placeholder="Enter documents" class="form-control" name="documents" required autofocus required="">
                        </div>
                        <div class="form-group">
                            <label>Images</label>
                            <input id="images" value="{{ old('images')}}" type="text" placeholder="Enter images" class="form-control" name="images" required autofocus required="">
                        </div>
                    </div>
                    <div class="form-group">
                        <label>Document Types</label>
                        <input id="document_types" value="{{ old('document_types')}}" type="text" placeholder="Enter document_types" class="form-control" name="document_types" required autofocus required="">
                    </div>
                    <div class="form-group">
                        <label>Unit No</label>
                ...
                <div class="tile-footer text-left mt-2">
                    <a class="btn btn-default" href="{{route('building_documents')}}">Cancel</a>
