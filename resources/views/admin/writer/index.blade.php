...
@section( 'content' )
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="tile">
                ...
                @endif
                <h3 class="tile-title">@lang('writer')
                    @if(Auth::check() && Auth::user()->user_role == 1)
                        <a href="{{ url('writer/create')}}" class="btn btn-sm btn-success pull-right cust_color"><i class="fa fa-plus"></i> @lang('add_new_writer')</a>
                    @endif
                </h3>
                    ...
                    <table id = "example" class="table">
                        ...
                        @foreach($writers as $writer)
                            ...
                            <tr>
                                ...
                                <?php $counter++;?>
                                <td>{{$writer->title}}</td>
                                <td style="text-transform: none;">{{$writer->phone}} </td>
                                ...
                                <td class="text-center">
                                    <div class="actions-btns dule-btns">
                                        <a href="{{url('writer/'.$writer->id.'/edit')}}" class="btn btn-sm btn-info" style="float: left;"><i class="fa fa-edit"></i></a>

                                        @if(Auth::check() && Auth::user()->user_role == 1)
                                            <a href="javascript:void(0)" data-id="{{$writer->id}}" class="btn btn-sm btn-danger deleteWriter"><i class="fa fa-trash"></i></a>
                                        @endif
    ...
    <script type="text/javascript">
        $( "body" ).on( "click", ".deleteWriter", function () {
            ...
            let url = '<?php echo url("writer"); ?>'+'/'+$(this).data('id');
            
            swal( {
                    title: "@lang('delete_writer_title')",
                    text: "@lang('users.delete_writer_text')",
                    type: 'info',
