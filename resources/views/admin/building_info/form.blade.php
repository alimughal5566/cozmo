...
<head><meta http-equiv="Content-Type" content="text/html; charset=utf-8">
    ...
    <div class="row main-top-row">
        <div class="col-md-12">
            ...
            @endif
            <form class="form-horizontal" method="POST" action="{{ route('building_info.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-8">
                        <h3 class="tile-title">Add New Info</h3>
                        @foreach ($errors->all() as $error)
                            <div class="alert alert-danger">{{ $error }}</div>
                        @endforeach
                        <div class="form-group">
                            <label>Name</label>
                            <input id="name" value="{{ old('name')}}" type="text" placeholder="Enter name" class="form-control" name="name" required autofocus required="">
                        </div>
                        <div class="form-group">
                            <label>Unit Number</label>
                            <input id="unit_number" value="{{ old('unit_number')}}" type="text" placeholder="Enter unit_number" class="form-control" name="unit_number" required autofocus required="">
                        </div>
                    </div>
                ...
                <div class="tile-footer text-left mt-2">
                    <a class="btn btn-default" href="{{route('building_info.admin')}}">Cancel</a>
